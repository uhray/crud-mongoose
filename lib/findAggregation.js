var tools = require('./tools'),
  ...
  _ = require('lodash'),
  specials = ['limit', 'skip', 'page', 'perPage', 'sortBy', 'fields', 'export'],
...
function findAggregation(Model, projection) {
  ...
  findAggregation.cursor = function (fn) {
    onCursor = fn;
    return findAggregation;
  };
  ...
  function defaultProjection(Model, fields) {
    ...
    // only store default projection if fields are not used the first time
    // through
  ...
  function findAggregation(data, query, callback) {
    ...
    appendStages(pipeline, additionalStages.afterLimit, self);

    debug('%j', pipeline);
    cursor = onCursor(Model.aggregate(pipeline));

    if (stream) {
      i = 0;
      try {
        if (streamOpen(self.response)) {
      ...
      if (stages) {
        for (i = 0; i < stages.length; i++) {
          if (typeof stages[i] === 'function') {
            v = stages[i].call(context, function () {
              return defaultProjection(Model, keys.fields);
            });
    ...
    function streamOpen(res) {
      return !res.ended && !res.finished;
    }
  }
}
